...
- **Railway is not picking up pushes to main.** The Bible-order sorting for the plan catalog was pushed 2026-08-06 and still is not live; Acts sits at the bottom of the shelf instead of between John and Romans, which is the old insertion-date sort. Ruled out on our side: the commit is on origin/main, main is the only branch, the build passes and all 19 server tests pass. Needs someone to open the Railway dashboard, check the service's deploy history, and either redeploy or turn auto-deploy back on. Only the ordering is waiting on it; all plan content is live already because it comes from the database.
  ...
  **How to check whether a deploy landed, as of 2026-08-24:** open
  `https://ironsharp-production.up.railway.app/health` in any browser. It now
  reports the commit it's running and when it last started, so a stuck
  auto-deploy no longer looks identical to a working one. Compare `commit`
  against the newest hash on main. `startedAt` moves on every redeploy even when
  the commit doesn't, which separates "Railway ignored the push" from "Railway
  redeployed and was already current."
